...
function Scene({
  ...
  useEffect(() => {
    const apply = () => {
      ...
      planeRef.current.material.uniforms.uInverted.value = isDark ? 1 : 0;
    };
  ...
  useFrame((state, delta: number) => {
    ...
    u.uAspect.value = Number.isFinite(aspect) && aspect > 0 ? aspect : 1;
  ...
  const uniforms = useMemo(() => {
    ...
    return {
      ...
      uAspect: new THREE.Uniform(viewport.width / viewport.height || 1),
    };
  }, [perlinNoiseTexture, offsets, viewport.width, viewport.height]);

  return (
    <mesh ref={planeRef} scale={[viewport.width, viewport.height, 1]}>
...
void main() {
  ...
  vec2 adjustedUv = vec2(centeredUv.x * uAspect, centeredUv.y);
  float maxRadius = length(vec2(uAspect, 1.0));

  // Convert uv to polar coordinates
  float radius = length(adjustedUv) / maxRadius;
  float theta = atan(adjustedUv.y, adjustedUv.x);
    ...
    // Blend a background gradient so the canvas fills the container
    float bgT = clamp((centeredUv.y + 1.0) * 0.5, 0.0, 1.0);
    bgT = mix(bgT, 1.0 - bgT, uInverted);
    vec3 background = mix(uColor1, uColor2, bgT);
    float detailAlpha = color.a * uOpacity;
    vec3 composed = mix(background, color.rgb, detailAlpha);

    gl_FragColor = vec4(composed, uOpacity);
}
`;
