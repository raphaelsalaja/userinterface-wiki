...
<Figure>
  <ModesDemo />
  <Caption>Compare sync, wait, and popLayout modes.</Caption>
</Figure>

## sync (default)

Both entering and exiting elements animate at the same time. You are responsible for handling any layout conflicts, typically with `position: absolute` or similar techniques.

## wait

The entering element waits until the exiting element has fully animated out. This creates sequential animations, perfect for when you want users to focus on one piece of information at a time.

```tsx
<AnimatePresence mode="wait">
  <motion.div key={currentStep} exit={{ opacity: 0 }} />
</AnimatePresence>
```

A tip: try `ease: "easeIn"` on the exit and `ease: "easeOut"` on the entrance. Together they create an overall `easeInOut` feel across the transition.

## popLayout

Exiting elements are "popped" out of the document flow, allowing surrounding elements to reflow immediately. This pairs exceptionally well with the `layout` prop for smooth list reordering.
